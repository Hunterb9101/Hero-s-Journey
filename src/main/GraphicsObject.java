...
public abstract class GraphicsObject {
	...
	public GraphicsObject(int iX, int iY, int iWidth, int iHeight, menuItem iParentMenu) {
		x = iX;
		y = iY;
		width = iWidth;
		height = iHeight;
	...
	public static void checkOnClick(int x, int y) {
		...
		for (int i = 0; i < allObjects.size(); i++) {
			...
			if ((x > obj.x * xScalar && x < (obj.width + obj.x) * xScalar)
				...
				if (obj.parentMenu.equals(Main.currentMenu) || obj.parentMenu.equals(Main.menuItem.NONE)) {
					obj.onClick();
				}
			}
		}
